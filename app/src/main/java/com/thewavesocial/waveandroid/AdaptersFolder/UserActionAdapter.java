            ...
            if (notifList.get(position).getRequestType() == Notification.TYPE_INVITE_GOING || notifList.get(position).getRequestType() == Notification.TYPE_INVITE_BOUNCING) {
                ...
                holder.accept.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        server_manageUserForParty(CurrentUser.getUser().getId(), notifList.get(position).getSenderID(), type, "POST", new OnResultReadyListener<String>() {
                            @Override
                            public void onResultReady(String result) {
                                if (result.equals("success")) {
                                    server_createNotification(CurrentUser.getUser().getId(), notifList.get(position).getSenderID(), notifList.get(position).getSenderID(), type, new OnResultReadyListener<String>() {
                                        ...
                                        public void onResultReady(String result) {
                                            if (result.equals("success")) {
                                                server_deleteNotification(CurrentUser.getUser().getId(), notifList.get(position).getId(), new OnResultReadyListener<String>() {
                                                    @Override
                                                    public void onResultReady(String result) {
                                                        if (result.equals("success")) {
                                    ...
                                    server_uninviteUser(CurrentUser.getUser().getId(), notifList.get(position).getSenderID(), null);
                                } else
                                    UtilityClass.endProgressbar(mainActivity, false);
                            }
                        });
